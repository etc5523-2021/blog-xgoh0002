...
```{r}
table5 <- total %>%
  select(Date, ConfirmedCovidCases, Male, Female, Unknown) 
table5[is.na(table5)] = 0
  
```

```{r}
covid <- cbind(table5$Male, table5$Female, table5$Unknown, table5$ConfirmedCovidCases)
```


```{r}
table5$Date <- ymd_hms(table5$Date)
```

```{r}
gg <- xts(x = covid, order.by = table5$Date)
```


```{r}
x <- dygraph(gg)%>% dyRangeSelector()
x
```
